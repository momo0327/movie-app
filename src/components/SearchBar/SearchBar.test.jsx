...
describe(SearchBar, () => {
  ...
  it('should tell the user when a movie is found', async() => { //testar meddelande att filmen hittas
    ...
    //enter
    expect(await screen.findByText("the shawshank redemption", {exact: false})).toBeInTheDocument();
  })
  ...
  it('should tell the user when a movie is not found', async () =>{ //testar felmeddelande
    ...
    //enter
    expect(await screen.findByText('no movies found', {exact: false})).toBeInTheDocument();
  })
});